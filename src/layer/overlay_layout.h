...
struct Metrics {
    ...
    int chat_stack_gap = 0;
    int chat_row_height = 0;
    int chat_row_gap = 0;
    int chat_pad_x = 0;
    int chat_pad_y = 0;
    int chat_radius = 0;
...
inline Metrics metrics(float scale) {
    ...
    m.chat_stack_gap = scaled_px(scale, 10);
    m.chat_row_height = scaled_px(scale, 28);
    m.chat_row_gap = scaled_px(scale, 4);
    m.chat_pad_x = scaled_px(scale, 10);
    m.chat_pad_y = scaled_px(scale, 8);
    m.chat_radius = scaled_px(scale, 8);
    m.chat_line_gap = scaled_px(scale, 3);
    m.chat_font_px = scaled_px(scale, 12);
    m.chat_max_text_width = scaled_px(scale, 360);
