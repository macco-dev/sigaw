...
- On Proton/Wine, `SIGAW_WINE_POLICY=auto|force|disable` controls Vulkan fallback behavior: `auto` uses a conservative present path, `force` keeps the more aggressive semaphore-forwarding path for debugging, and `disable` is the safe escape hatch if an injected Vulkan layer keeps a game from launching.
